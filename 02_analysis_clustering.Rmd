...
clara_result = phosp_3m %>% 
  ...
  prep() %>% 
  juice() %>% 
  mutate(across(c(mocal_total, sppb_score), ~ . * -1)) %>% 
...
phosp_3m %>% 
  ...
  mutate(
    key = factor(key) %>% 
      fct_recode(
        ...
        "Dyspnoea 12" = "dyspnoea12_summary",
        "Function (FACIT)" = "facit_item_total",
        "Physical (SPPB)" = "sppb_score",
      )
  ) %>% 
  ...
  guides(colour = guide_legend(title.position = "top",
                               nrow = 2)) + 
  ggtitle("Symptom instruments by MOCA",
          " ")
```
...
phosp_hosp %>% 
  ...
  summary_factorlist(dependent,  explanatory, 
                     na_include = TRUE, cont = "median", p = TRUE,
