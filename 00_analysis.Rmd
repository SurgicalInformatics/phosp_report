...
```{r}
explanatory_pft = phosp_3m_pft %>% 
  select(-c(study_id, pft_spm_done, pft_spm_reason_not_done, pft_spm_date, pft_tlco_date, pft_kco_date, 
            pft_mip_date, pft_mep_date)) %>% 
  names()

phosp_3m_pft %>%
  left_join(phosp_hosp %>% select(study_id, crf1a_resp_support_4levels)) %>% 
  mutate(across(c(pft_tlco_reading1, pft_tlco_reading2, pft_kco_reading1, pft_kco_reading2, 
                  pft_mip_reading, pft_mip_reading2, pft_mep_reading1, pft_mep_reading2), as.numeric)
  ) %>% 
  ff_relabel_df(phosp_3m_pft) %>% 
